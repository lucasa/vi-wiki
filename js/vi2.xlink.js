...
//			$(this.options.target_selector).parent().append($('<div></div>').attr('id', this.options.selector));
		...
		buildLinkList : function(ann){
			...
			$.each(ann, function(i, val){
				if(val.linktype == 'cycle' || val.linktype == 'standard' || val.linktype == 'xlink' || val.linktype == 'external'){ 
...
/*			if(this.currLinkId == id){
		...
		relativePos : function(obj){  
			...
			return {x: Math.floor((obj.x/100) * main.options.videoWidth), y: Math.floor((obj.y/100) * main.options.videoHeight)};
		},
