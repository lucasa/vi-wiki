...
//alert($(this.selector).text().split(/\n/)[1])
  			$.each($(this.selector).text().split(/\n/), function(i, val){  
  		...
  		parseWikiVideo : function(str){
  				...
  				$.each(a, function(i, val){
  					...
  					else if(val.match(/(?=.webm)/)){ url = val; }  /////////////////////////////////// bugy ... only one type
  					else if(val.length > 0){ id = val; }
  				})
  				//alert(str+'   url:'+url +' start:'+ start +'  duration:'+ duration +'  id:'+ id);
  				// build arr
  				arr['id'] = id;
  				arr['url'] = url;
  		...
  		parseWikiHyperlink : function(str){ 
  				...
  				obj.t2 = str.match(re) ? (new String(re.exec(str2[1]))).replace(/[\|]/, '') : 1000;
					...
					//alert('spatial: '+obj.x+' - '+obj.y+'   temporal: '+obj.t1+' - '+obj.t2);
